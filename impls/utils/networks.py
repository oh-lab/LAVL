...
class MLPwEncoderValue(nn.Module):
    hidden_dims: Sequence[int]
    latent_dim: int
    layer_norm: bool = True
    encoder: nn.Module = None
    ...
    def setup(self):
        self.phi = MLP((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
        self.mlp = MLP((*self.hidden_dims, 1), activate_final=False, layer_norm=self.layer_norm)
    ...
    def __call__(self, observations, goals, is_phi=False, info=False):
        ...
        v = nn.softplus(self.mlp(jnp.concatenate([phi_s, phi_g], axis=-1))).squeeze(-1)
...
class LANValue(nn.Module):
    ...
    def __call__(self, observations, goals, is_phi=False, info=False):
        if is_phi:
            ...
            phi_g = self.phi_goal(goals)

        d = jnp.sum((phi_s - phi_g) ** 2, axis=-1)
        d = jnp.sqrt(d + 1e-6)

        if info:
            return d, phi_s, phi_g
        else:
            return d

class HilbertValue(nn.Module):
    hidden_dims: Sequence[int]
    latent_dim: int
    layer_norm: bool = True
    encoder: nn.Module = None
